...
let apiTasksLogsId, logsEdit, apiAssets;
...
// Page-Log Reservation Edit
describe("Page-Log Reservation Edit", () => {
  const keysAsset = ["status", "assetId", "alias", "typeId", "organization", "region", "location"];
  const keysAssetApi = ["status", "assetId", "alias", "typeId", "customerId", "regionId", "locationId"];
  let page = 0;
  const timeZone = `GMT${Cypress.moment().format("Z")}`;
  before(() => {
    apiTasksLogsId = Cypress.env("apiTasksLogsId");
    logsEdit = Cypress.env("logsEdit");
  ...
  context("Initial display", () => {
    it("Initial display", () => {
      ...
      cy.server()
        .route("GET", `${apiAssets}*`)
        .as("apiAssets")
        .route("GET", `${apiTasksLogsId}*`)
        .as("apiTasksLogsId");
      cy.visit(Cypress.env("logsEdit"))
        .wait(["@apiAssets", "@apiTasksLogsId"])
        .then((api) => {
          task.assets = api[1].response.body.assets;
          cy.request("GET", `${Cypress.env("apiTasksLogsId")}`).then((res) => {
            const { body } = res;
            ...
            cy.get("bridge-table[data-test='asset-selected-table'] tbody tr").each(($el, i) => {
              keysAssetApi.forEach((value, index) => {
                switch (value) {
                  case "status":
                    cy.wrap($el)
            ...
            // check value memo
            cy.get("bridge-log-reservation-edit-page")
            ...
            // Date value check
            cy.get("bridge-log-reservation-edit-page .download-setting")
  ...
  context("Target Asset operations", () => {
    it("Click the Select assets button", () => {
      cy.server()
        .route("GET", `${apiAssets}*`)
        .as("apiAssets");
      cy.visit(Cypress.env("logsEdit")).wait("@apiAssets");
      cy.get("bridge-log-reservation-edit-page bridge-button[data-test='asset-modal'].basic button")
        .click()
        .then(() => {
          cy.wait(1000);
            ...
            .then(() => {
              ...
              cy.get(".cdk-overlay-container bridge-table-board bridge-table tbody tr").then(($el) => {
                if (Cypress.$($el).length > 2) {
                  cy.get(".cdk-overlay-container bridge-table-board bridge-table tbody tr:eq(2)")
                    .click()
                    .then(() => {
                      cy.wait(500);
                      cy.get(".cdk-overlay-container bridge-table-board bridge-table tbody tr:eq(2) td:eq(0) mat-checkbox")
                        .click()
                        .then(() => {
                          const data = {};
                          cy
                            .get(".cdk-overlay-container bridge-table-board bridge-table tbody tr:eq(2)")
                            .find("td")
                            .each(($td, index) => {
                              if (index > 0) cy.wrap($td).within((t) => (data[keysAssetApi[index - 1]] = t.text()));
                            }),
  ...
  context("Retrieve Log Setting operations", () => {
    before(() => {
      cy.visit(Cypress.env("logsEdit"));
    });
      ...
      cy.get("bridge-log-reservation-edit-page")
        ...
        .within(() => {
          cy.root()
            ...
            .click({ force: true })
            .then(() => {
              cy.location("href").should("eq", Cypress.config("baseUrl") + Cypress.env("logsEdit"));
            });
        });
    });
  });
  context("Confirm operation", () => {
    beforeEach(() => {
      cy.visit(Cypress.env("logsEdit"));
    });
    ...
    it("Select Target Asset, Start at Immediately", () => {
      // Select Target Asset
      cy.get("bridge-log-reservation-edit-page bridge-button[data-test='asset-modal'].basic button")
        .click()
        .then(() => {
...
describe("Page-Log Reservation,Select Assets", () => {
                ...
                .then((body) => {
                  ...
                  cy.get("bridge-side-board")
                    ...
                    .then(() => {
                      // check length list
                      cy.get(`.cdk-overlay-container .mat-select-panel  mat-option`).then(($el) => {
                        cy.wrap($el).should("have.length", body.length, 1);
                      });
                    });
                });
            });
              ...
              cy.get(`.cdk-overlay-container .mat-select-panel  mat-option:eq(1)`)
                ...
                .then(() => {
                  ...
                  cy.get("bridge-select[data-test='organization-select'] mat-select")
                    .invoke("text")
                    .then((text) => text.trim())
                    .should("eq", res.body[0].customerId);
                  cy.get("bridge-side-board")
                    .find("bridge-select[data-test='location-select'] mat-select")
                    .should("have.attr", "ng-reflect-value", ``)
                    .click()
  ...
  context("Sorting operations", () => {
    before(() => {
      cy.server()
        .route("GET", Cypress.env("apiTasksLogsId"))
        .as("apiTasksLogsId");
      cy.visit(Cypress.env("logsEdit")).wait("@apiTasksLogsId");
    ...
    sourceSort.forEach(({ key, field }) => {
      // sort ASC
      it(`Click ${field} to set the ascending order`, () => {
        cy.server()
      ...
      // sort DESC
      it(`Click ${field} to set the descending order`, () => {
        cy.server()
          .route("GET", `${Cypress.env("apiAssets")}*`)
          .as(`sort${key}`);
        cy.wait(1000).then(() => {
      ...
      // sort default
      it(`Click ${field} to set the default order`, () => {
        cy.server()
          .route("GET", `${Cypress.env("apiAssets")}*`)
          .as(`sort${key}`);
        cy.wait(1000).then(() => {
  ...
  context("Paging operations", () => {
    ...
    handleClickPage.forEach(({ action, alias }) => {
      it(`Click the ${action} button`, () => {
        let totalCount = 0;
        cy.request("GET", `/api/assets?isFilter=false&limit=10&offset=0`).then((res) => {
          ...
          if (numberPage > 3) {
            cy.get("bridge-modal bridge-pagination")
              ...
              .within(() => {
                cy.get("nav.pagination").scrollIntoView();
                if (`${action}` === "page 2") {
                  cy.server()
                    .route("GET", `/api/assets?sort=desc&isFilter=false&limit=10&offset=10`)
                    .as(`paging${alias}`);
                  ...
                  cy.server()
                    .route("GET", `/api/assets?sort=desc&isFilter=false&limit=10&offset=20`)
                    .as(`paging${alias}`);
                  cy.get("div.root > nav > button")
                    .last()
                    .click();
        ...
        // CheckBox, Status, Serial, Name, Model, Organization, Region, Location
        cy.wait(`@paging${alias}`).then(({ responseBody }) => {
          let resData = responseBody.items;

          cy.get("bridge-table-board > div > bridge-table > table > tbody > tr")
            .first()
            .within(() => {
              cy.get("bridge-badge .badge").should("have.text", `${resData[0].status}`);
              cy.get("div[data-test='table-serial']")
                .first()
                .invoke("text")
                .then((text) => text.trim())
                .should("eq", `${resData[0].assetId}`);
            });

          cy.get("bridge-table-board > div > bridge-table > table > tbody > tr")
            .first()
            .find("td.tableCell-name")
            .invoke("text")
            .then((text) => text.trim())
            .should("eq", `${resData[0].alias}`);
          ...
          cy.get("bridge-table-board > div > bridge-table > table > tbody > tr")
            ...
            .invoke("text")
            .then((text) => text.trim())
            .should("eq", `${resData[0].typeId}`);

          cy.get("bridge-table-board > div > bridge-table > table > tbody > tr")
            .first()
            .find("td.tableCell-organization")
            .invoke("text")
            .then((text) => text.trim())
            .should("eq", `${resData[0].organization}`);

          cy.get("bridge-table-board > div > bridge-table > table > tbody > tr")
            .first()
            .find("td.tableCell-region")
            .invoke("text")
            .then((text) => text.trim())
            .should("eq", `${resData[0].region}`);

          cy.get("bridge-table-board > div > bridge-table > table > tbody > tr")
            .first()
            .find("td.tableCell-location")
            .invoke("text")
            .then((text) => text.trim())
            .should("eq", `${resData[0].location}`);
        });
      });
    });
  });
  ...
  context("Cancel operations", () => {
    it(`Click the Cancel button`, () => {
      ...
      cy.visit(Cypress.env("logsEdit"))
        .wait("@visitTaskLogs")
        .then(({ responseBody }) => {
          cy.get("div[data-test='asset-select'] bridge-table > table > tbody > tr").should(($tr) => {
            expect($tr).to.have.length(responseBody.assets.length);
          });
          ...
          cy.get("mat-dialog-actions > bridge-button.basic button")
            .click()
            .then(() => {
              cy.wait(100);
              cy.get("div[data-test='asset-select'] bridge-table > table > tbody > tr").should(($tr) => {
      ...
      cy.visit(Cypress.env("logsEdit"))
        ...
        .then(({ responseBody }) => {
          ...
          cy.get("bridge-button[data-test='asset-modal']")
            ...
            .should("be.visible")
            .click();
          cy.get("bridge-table-board bridge-table > table > tbody > tr > td.tableCell-checked")
            .eq(1)
            .click();
          cy.get("mat-dialog-actions > bridge-button.primary button")
            .click()
            .then(() => {
              cy.wait(100);
              cy.get("div[data-test='asset-select'] bridge-table > table > tbody > tr").should(($tr) => {
                expect($tr).not.to.have.length(responseBody.assets.length);
              });
            });
        });
    });
