...
let apiTasks, apiTasksReboots, rebootNew;
...
describe("Page-Reboot Reservation", () => {
  ...
  before(() => {
    ...
    apiTasksReboots = Cypress.env("apiTasksReboots");
    rebootNew = Cypress.env("rebootNew");
    apiAssets = Cypress.env("apiAssets");
    cy.server()
      .route("GET", `${apiTasksReboots}*`)
      .as("apiTasksReboots")
      .route("GET", `${rebootNew}*`)
      .as("rebootNew");
  });
  ...
  context("initial display", () => {
    it("Displayed when transitioning from the Task List", () => {
      ...
      cy.get("bridge-task-list-page bridge-button.mat-menu-trigger button")
        ...
        .then(($el) => {
          cy.wait(500);
          cy.get(`.cdk-overlay-container .mat-menu-panel bridge-dropdown-menu-item:nth-child(3) div`)
            .click()
            .then(() => {
              cy.wait(500);
              cy.request("GET", rebootNew).then(() => {
                cy.wait(100);
                ...
                cy.get("bridge-reboot-reservation-page")
                  .find("fieldset form textarea[data-test='memo']")
                  .should("have.value", "");

                // Start at value check (Immediately)
                cy.get("bridge-reboot-reservation-page .task-setting")
                  .find("bridge-radio")
                  .first()
      ...
      cy.visit(Cypress.env("assetDetail"))
        ...
        .then((api) => {
          ...
          cy.get("bridge-detail-basis-board")
            .find("bridge-card[data-test='asset-basis'] bridge-button button:eq(0)")
            .click()
            .then(() => {
              cy.wait(500);
    ...
    it("Displayed when transitioning from the Confirmation", () => {
      ...
      cy.server()
        .route("GET", `${apiAssets}*`)
        .as("apiAssets");
      cy.visit(Cypress.env("rebootNew")).wait("@apiAssets");
      cy.get("bridge-reboot-reservation-page bridge-button[data-test='asset-modal'].basic button")
        .click()
        .then(() => {
          cy.wait(500);
                                    ...
                                    cy.get(`.cdk-overlay-container bridge-time-picker-content mat-option`)
                                      ...
                                      .then((value) => {
                                        cy.get("bridge-reboot-reservation-page")
                                          ...
                                          .then(() => {
                                            cy.wait(200);
                                            cy.get("bridge-reboot-reservation-confirmation-page")
  ...
  context("Target Asset operations", () => {
    it("Click the Select assets button", () => {
      cy.visit(Cypress.env("rebootNew"));
      cy.get("bridge-reboot-reservation-page bridge-button[data-test='asset-modal'].basic button")
        .click()
        .then(() => {
          cy.wait(500);
  ...
  context("Reboot Setting operations", () => {
    before(() => {
      cy.visit(Cypress.env("rebootNew"));
    });
...
describe("Page-Reboot Reservation,Select Assets", () => {
  ...
  context("Sorting operations", () => {
    before(() => {
      cy.visit(Cypress.env("rebootNew"));
      cy.get("bridge-reboot-reservation-page bridge-button[data-test='asset-modal'].basic button").click();
    });
