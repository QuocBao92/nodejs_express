...
let apiTasks, apiTasksReboots, rebootEdit, apiTasksRebootsId;
...
let data;
describe("Page-Reboot Reservation Edit", () => {
  ...
  context("initial display", () => {
    it("initial display", () => {
      ...
      cy.visit(Cypress.env("rebootEdit"))
        .wait("@apiTasksRebootsId")
        .then((api) => {
          task.assets = api.response.body.assets;
          cy.request("GET", `${Cypress.env("apiTasksRebootsId")}`).then((res) => {
            const { body } = res;
            ...
            // Date value check
            cy.get("bridge-reboot-reservation-edit-page .download-setting")
  ...
  context("Target Asset operations", () => {
    ...
    it.only("Displayed after an asset is selected, 0 selections", () => {
      cy.server()
        .route("GET", `${apiTasksRebootsId}*`)
        .as("apiTasksRebootEditId");
      cy.visit(Cypress.env("rebootEdit"))
        .wait("@apiTasksRebootEditId")
        .then(() => {
          cy.wait(1000);
          cy.get("bridge-button[data-test='asset-modal'].basic button")
            .click()
      ...
      cy.get("bridge-reboot-reservation-edit-page")
        ...
        .within(() => {
          cy.root()
            ...
            .click()
            .then(() => {
              cy.location("href").should("eq", Cypress.config("baseUrl") + Cypress.env("rebootEdit"));
            });
        });
    });
  });
...
describe("Page-Reboot Reservation,Select Assets", () => {
  ...
  context("Sorting operations", () => {
    before(() => {
      cy.visit(Cypress.env("rebootEdit"));
      cy.get("bridge-reboot-reservation-edit-page bridge-button[data-test='asset-modal'].basic button").click();
    });
