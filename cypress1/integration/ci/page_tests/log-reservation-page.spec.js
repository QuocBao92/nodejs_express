...
describe("Page-Log Reservation", () => {
  ...
  context("initial display", () => {
    it("Displayed when transitioning from the Task List", () => {
      ...
      cy.get("bridge-task-list-page bridge-button.mat-menu-trigger button")
        ...
        .then(() => {
          cy.wait(500);
          cy.get(`.cdk-overlay-container .mat-menu-panel bridge-dropdown-menu-item:eq(1) div`)
            .click()
            .then(() => {
              cy.wait(500);
              ...
              cy.get("bridge-log-reservation-page .download-setting")
                .find("bridge-radio")
                .first()
      ...
      cy.visit(Cypress.env("assetDetail"))
        ...
        .then((api) => {
          ...
          cy.get("bridge-detail-basis-board")
            .find("bridge-card[data-test='asset-basis'] bridge-button button:eq(3)")
            .click()
            .then(() => {
              cy.wait(500);
          ...
          cy.get(".cdk-overlay-container bridge-table-board bridge-table tbody tr").then(($el) => {
            if (Cypress.$($el).length > 1) {
              ...
              cy.get(".cdk-overlay-container mat-dialog-container mat-dialog-actions bridge-button")
                ...
                .then(() => {
                  cy.wait(500);
                  cy.get("bridge-log-reservation-page bridge-table[data-test='asset-selected-table'] tbody")
                    .find("tr")
                    .its("length")
                    .should("eq", 1);
                  //Display check (display) of the number of items in the Target Asset list
                  cy.get("bridge-log-reservation-page bridge-table[data-test='asset-selected-table'] tbody tr").each(($el, i) => {
                    keysAsset.forEach((value, index) => {
                      switch (value) {
                        case "status":
                          cy.wrap($el)
                  ...
                  cy.get("bridge-log-reservation-page bridge-pagination[data-test='asset-selected-pagination']")
                    .find("span.description")
                    .invoke("text")
                    .then((text) => text.trim())
                    .should("equal", `1 to 1 of 1 items`);
  ...
  context("Log Setting operations", () => {
    before(() => {
      cy.visit(Cypress.env("logsNew"));
    });
                        ...
                        cy.get("bridge-log-reservation-page")
                          ...
                          .then(() => {
                            ...
                            cy.get(
                              ...
                              .click()
                              .then(() => {
                                cy.get("bridge-time-picker bridge-button button")
                                  .click()
                                  .then(() => {
  ...
  context("Sorting operations", () => {
    before(() => {
      cy.server()
        ...
        .as("apiAssets");
      cy.visit(Cypress.env("logsNew")).wait("@apiAssets");
      cy.get("bridge-log-reservation-page bridge-button[data-test='asset-modal'].basic button").click();
    });
