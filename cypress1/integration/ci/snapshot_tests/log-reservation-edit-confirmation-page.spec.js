describe("Snapshot-testing: Log Reservation Edit Confirmation", () => {
  const viewports = [
      ...
      fixtures.forEach((fixture) => {
        it(`Case: ${fixture} data`, () => {
          ...
          cy.server()
            ...
            .route(
              "GET",
              `${Cypress.env("apiTasksLogsIdRL").replace("${1}", apiParams.tasksLogsId.taskId)}*`,
              `fx:tasks/logs-id/${fixture}`,
            )
            .as("getTasksLogsId");
          cy.visit(Cypress.env("logsEditRL").replace("${1}", urlParams.taskId)).wait(
            ["@getAssets", "@getTypes", "@getRegions", "@getCustomers", "@getTasksLogsId"],
