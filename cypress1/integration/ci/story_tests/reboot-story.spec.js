describe("Story - Task Reboot", () => {
  let page = `${Cypress.env("tasks")}`;
  context("Registering Reboot task from the Task List", () => {
    beforeEach(() => {
      cy.server()
    ...
    it("Click the Cancel button in Create New Reboot to go to Task List", () => {
      cy.wait(500);
      cy.get("bridge-task-list-page bridge-task-list-template .matMenu bridge-button button")
        .click()
        .then(() => {
          cy.wait(500);
          cy.get(".cdk-overlay-container .mat-menu-content bridge-dropdown-menu-item:eq(2) > div")
            .click()
            .then(() => {
              cy.wait(500);
    ...
    it("Click the Back button in Confirmation to go to Create New Reboot", () => {
      cy.wait(500);
      cy.get("bridge-task-list-page bridge-task-list-template .matMenu bridge-button button")
        .click()
        .then(() => {
    ...
    it("Register Task", () => {
      cy.wait(500);
      const taskName = "New Reboot - " + new Date().getTime();
      const task = {
        taskName,
        download: "",
        assets: [],
  ...
  context("Registering Reboot task from the Asset Detail", () => {
    beforeEach(() => {
    ...
    it("Click the Cancel button in Create New Reboot to go to Task List", () => {
      cy.wait(500);
      cy.get(
        "bridge-asset-detail-page bridge-asset-detail-template bridge-detail-basis-board bridge-card ul.none-list li:eq(0) bridge-button",
      )
        .click()
        .then(() => {
          cy.wait(500);
  ...
  context("Editing Reboot task", () => {
    ...
    it("Click the Cancel button in Edit Reboot to go to Task List", () => {
      cy.wait(500);
            ...
            cy.wrap($panel)
              ...
              .then(() => {
                cy.location("href").then((url) => {
                  ...
                  const urlEdit = Cypress.env("apiTasksRebootsIdRL").replace("${1}", taskId);
    ...
    it("Click the Back button in Confirmation to go to Edit Reboot", () => {
      cy.wait(500);
      cy.get("bridge-task-list-page bridge-task-list-template").then(($el) => {
        cy.wrap($el)
          .find("bridge-task-table mat-accordion > [data-test='task-row']:eq(0) mat-expansion-panel-header")
            ...
            cy.wrap($panel)
              ...
              .then(() => {
                cy.location("href").then((url) => {
                  const arrayLink = url.split("/");
                  const taskId = arrayLink[arrayLink.length - 1];
                  const apiTasksRebootsIdRL = Cypress.env("apiTasksRebootsIdRL").replace("${1}", taskId);
                  cy.request(apiTasksRebootsIdRL).then(({ body }) => {
                    cy.wait(500);
                    cy.get("bridge-button[data-test='confirm-button']")
                      .click()
                      .then(() => {
                        ...
                        cy.get("bridge-button[data-test='back-button']")
                          ...
                          .then(() => {
                            cy.wait(500);
                            const rebootEditRL = Cypress.env("rebootEditRL").replace("${1}", body.id);
                            cy.location("href").should("eq", Cypress.config("baseUrl") + rebootEditRL);
                          });
                      });
                  });
                });
  ...
  context("Deleting Reboot task", () => {
    ...
    it("Delete Task", () => {
      cy.wait(500);
      cy.server()
        .route("DELETE", `/api/tasks/reboots/*`)
        .as("apiDeleteTasks");
      cy.request(`${Cypress.env("apiTasks")}?limit=10&offset=0`).then(({ body }) => {
