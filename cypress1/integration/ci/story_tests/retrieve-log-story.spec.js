describe("Story - Task Retrieve Log", () => {
  let page = `${Cypress.env("tasks")}`;
  context("Registering Retrieve Log task from the Task List", () => {
    beforeEach(() => {
      cy.server()
    ...
    it("Click the Cancel button in Create New Retrieve Log to go to Task List", () => {
      cy.wait(500);
      cy.get("bridge-task-list-page bridge-task-list-template .matMenu bridge-button button")
        .click()
        .then(() => {
          cy.wait(500);
          cy.get(".cdk-overlay-container .mat-menu-content bridge-dropdown-menu-item:eq(1) > div")
            .click()
            .then(() => {
              cy.wait(500);
    ...
    it("Click the Back button in Confirmation to go to Create New Retrieve Log", () => {
      cy.wait(500);
      cy.get("bridge-task-list-page bridge-task-list-template .matMenu bridge-button button")
        .click()
        .then(() => {
    ...
    it("Register Task", () => {
      cy.wait(500);
      const taskName = "New Retrieve Log - " + new Date().getTime();
      const task = {
        taskName,
        download: "",
        assets: [],
          ...
          cy.get(".cdk-overlay-container .mat-menu-content bridge-dropdown-menu-item:eq(1) > div")
            ...
            .then(() => {
              ...
              cy.get("form").then(($form) => {
                ...
                cy.wrap($form)
                  .find(".download-setting bridge-radio:eq(1)")
                  .click()
                  .then(() => {
                    cy.wrap($form)
  ...
  context("Registering  Retrieve Log task from the Asset Detail", () => {
    beforeEach(() => {
    ...
    it("Click the Cancel button in Create New Retrieve Log to go to Task List", () => {
      cy.wait(500);
      cy.get(
        "bridge-asset-detail-page bridge-asset-detail-template bridge-detail-basis-board bridge-card ul.none-list li:eq(3) bridge-button",
      )
        .click()
        .then(() => {
          cy.wait(500);
  ...
  context("Editing Retrieve Log task", () => {
    beforeEach(() => {
      cy.server()
        .route("GET", `${Cypress.env("apiTasks")}*`)
        .as("apiTasks")
    ...
    it("Click the Cancel button in Edit Retrieve Log to go to Task List", () => {
      cy.wait(500);
            ...
            cy.wrap($panel)
              ...
              .then(() => {
                cy.location("href").then((url) => {
                  ...
                  const urlEdit = Cypress.env("apiTasksLogsIdRL").replace("${1}", taskId);
    ...
    it("Click the Back button in Confirmation to go to Edit Retrieve Log", () => {
      cy.wait(500);
      cy.get("bridge-task-list-page bridge-task-list-template").then(($el) => {
        cy.wrap($el)
          .find("bridge-task-table mat-accordion > [data-test='task-row']:eq(0) mat-expansion-panel-header")
            ...
            cy.wrap($panel)
              ...
              .then(() => {
                cy.location("href").then((url) => {
                  const arrayLink = url.split("/");
                  const taskId = arrayLink[arrayLink.length - 1];
                  const apiTasksRetrieveLogsIdRL = Cypress.env("apiTasksLogsIdRL").replace("${1}", taskId);
                  cy.request(apiTasksRetrieveLogsIdRL).then(({ body }) => {
                    cy.wait(500);
                    cy.get("bridge-button[data-test='confirm-button']")
                      .click()
                      .then(() => {
                        ...
                        cy.get("bridge-button[data-test='back-button']")
                          ...
                          .then(() => {
                            cy.wait(500);
                            const retrieveLogsEditRL = Cypress.env("logsEditRL").replace("${1}", body.id);
                            cy.location("href").should("eq", Cypress.config("baseUrl") + retrieveLogsEditRL);
                          });
                      });
                  });
                });
  ...
  context("Deleting Retrieve Log task", () => {
    before(() => {
      cy.server()
        .route("GET", `${Cypress.env("apiTasks")}*`)
        .as("apiTasks")
